...
if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] == true) {
	...
	if($row["user_type"] == 'ADMIN'){
		header("Location: /phpregister/adminDashboard.php");
	}
} else {
								...
								<tbody>
									...
									while($row = $result->fetch_assoc()){
										...
										if($row["status"] == 'PENDING'){
											echo '<td><button class="action-button" data-id="' . $row["reg_id"] . '">WITHDRAW</button></td></tr>';
										} elseif ($row["status"] == 'APPROVED') {
											echo '<td><button style="background:green;" class="action-button" data-id="' . $row["reg_id"] . '">YOUR APPLICATION IS APPROVED</button></td></tr>';
										} elseif ($row["status"] == 'REJECTED') {
											echo '<td><button style="background:red;" class="action-button" data-id="' . $row["reg_id"] . '">YOUR APPLICATION IS REJECTED</button></td></tr>';
										}
									}
									?>
...
<script>
    ...
    document.querySelectorAll('.action-button').forEach(function(button) {
    	button.addEventListener('click', function() {
            ...
            } else if (this.textContent === 'WITHDRAW' && confirm("Are you sure you want to withdraw this application?")) {
            	window.location.href = "/phpregister/withdraw.php?id=" + id;
            }
        });
    });
</script>
